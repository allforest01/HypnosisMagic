...
struct FrameWrapper {
    ...
    bool scale_calculated, is_hovered, is_focused, textureGenerated, texturePushed;

    FrameWrapper():
        width(0), height(0), channels(0), scaled_width(0), scaled_height(0), start_x(0), start_y(0),
        scale_calculated(false), is_hovered(false), is_focused(false), textureGenerated(false), texturePushed(false) {}

    bool isTextureGenerated();
    bool isTexturePushed();
